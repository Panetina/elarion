...
- Player-facing Chronicle/library event families must provide at least 10
  authored stable variants, metadata requirements, missing-context fallback
  text, and tests before they are promoted as complete. This applies to future
  war, peace, revolution, story quest, seasonal event, NPC relationship, Realm,
  death, title, reward, mount, pet, and other public-history features. Variant
  selection must be stable per event through `chronicle.variant` metadata or a
  deterministic event/family selection; do not render truly random wording each
  time.
