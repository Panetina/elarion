...
| `src/main/java/panetina/elarion/addons/angling/model/AnglingFishingSession.java` | Angling sessions | Immutable compact active-session and retry-stable completion state | New Elarion code | `AnglingFishingSessionTest`, `AnglingFishingSessionServiceTest` |
...
| `src/main/java/panetina/elarion/addons/angling/service/AnglingFishingTriggerService.java` | Angling gameplay boundary | Replace stale cast state, complete the direct-player session, and cancel unfinished casts | New Elarion code | `AnglingFishingTriggerServiceTest` |
...
| `src/main/java/panetina/elarion/addons/angling/integration/VanillaFishingHooks.java` | Fabric fishing boundary | Convert server bobber fishing position/unload/retrieval into authoritative context and direct session operations | New Elarion code | `AnglingFishingTriggerServiceTest`, `:addons:angling:build` |
...
| `src/test/java/panetina/elarion/addons/angling/service/AnglingFishingTriggerServiceTest.java` | Angling tests | Validate stale-cast replacement, one-shot completion, unload cancellation, and no telemetry on cancellation | New Elarion code | `:addons:angling:test` |
...
| `src/main/resources/assets/elarion_angling/lang/en_us.json` | Resources | Placeholder translation namespace only | New Elarion code | `:addons:angling:processResources` |
| `src/main/resources/data/elarion_angling/angling/fish/placeholder_fish_001.json` | Angling definitions | Single neutral placeholder definition proving the packaged resource reload path | New Elarion content | `PlaceholderFishResourceTest` |
...
| Active fishing session | Player/session | No | Direct actor lookup plus bounded deadline priority queue | Driven by server bobber fishing tick, retrieval, and unload |
...
```text
Connected-player fishing smoke validation and reward delivery specification
```
...
one-shot per-cast selection, and rate-limited placeholder feedback exist. Next
validate the flow on a dedicated server and specify custom reward delivery
before items, inventory mutation, commands, networking, UI, titles, History
milestones, Chronicle integration, or copied content.
...
| Catch condition type | Angling gameplay | Pure/bounded condition evaluation with fail-closed unknown IDs | Implemented internally |
| Minigame behavior type | Angling gameplay | Server-approved behavior ID and result | Planned |
| Reward provider | Core rewards | Validated bounded reward output | Planned |
