...
final class AnglingSweetspotBehaviorParityTest {
    @Test
    void everyNativeSweetspotResolvesThroughFrozenBehaviorRegistry() throws Exception {
        var behaviors = AnglingSweetspotBehaviors.create();
        assertEquals(9, behaviors.registeredIds().size());
        ...
        for (Path path : definitions) {
            ...
            for (var sweetspot : definition.difficulty().sweetspots()) {
                behaviors.require(definitionId, sweetspot.sweetspotType());
                resolved++;
            }
        }
        assertEquals(352, resolved, "Frozen native sweetspot count changed");
    }

    private static Path resourcePath(String name) throws URISyntaxException {
        URL resource = Objects.requireNonNull(
                AnglingSweetspotBehaviorParityTest.class.getClassLoader().getResource(name),
