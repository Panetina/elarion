...
The approved roadmap. `TODO.md` is the only current action list. Every
milestone is implemented in focused, documented, tested, pushed commits.

## P0 — Stable Foundation and Release

| Milestone | Outcome | Exit gate |
| --- | --- | --- |
| P0.0 | Recover and integrate the existing WIP by domain. | Backups, focused commits, full build/GameTests/dedicated smoke test. |
| P0.1 | Make the Modpack release-safe. | World Reset, Backpacks, Guild/Chat, Government/NPC bridge and local QA are verified. |
| P0.2 | Make Platform staging usable. | Launcher-only website identity, simple whitelist, wiki/media and staging QA pass. |
...
- Modpack owns Minecraft/Fabric compatibility and the canonical release
  ...
  pointers. The website reads release metadata; it never hardcodes pack copies
  or Minecraft identifiers.
- Website identity uses a hidden Mojang UUID as canonical key and a verified
  Minecraft username as immutable visible login/display name until Fabric
  projects a chosen character name. The launcher proves ownership through a
...
- Introduce Chronicle allowlist-first destinations (`WEBSITE`, `LIBRARY_BOOK`,
  `NEWSPAPER`, `ADMIN_ONLY`), bounded dedupe/rate policy and shared filters by
  category, family, Realm, period and destination.
- Add the shared player-link/profile-open contract and apply Option A assets
  only when a consuming screen is implemented.

## P2 — Gameplay and Institutions

- Deliver Angling vertically and only open its public gate after dedicated and
  client QA.
- Build Ashen lifecycle with 90-day private recovery retention and audited,
  exceptional restoration.
- Implement the 2 Supporter / 1 normal admission scheduler, $5/month signed
...
- Player-facing history/search never scans raw JSONL, every world region or all
...
- Full exports are release/cross-module operations. Feature work exports only
  the changed module artifacts after hash verification.
